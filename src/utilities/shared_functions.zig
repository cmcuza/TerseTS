...
pub fn eliasGammaEncoder(values: []const u64, encoded_values: *ArrayList(u8)) !void {
...
test "eliasGammaEncoder can encode simple values correctly" {
    ...
    const encoded_values = ArrayList(u8).init(allocator);
    defer encoded_values.deinit();
    try eliasGammaEncoder(&vals, &encoded_values);
...
test "eliasGammaEncoder cannot encode zero value" {
    ...
    const encoded_values = ArrayList(u8).init(allocator);
    defer encoded_values.deinit();
    eliasGammaEncoder(&vals, &encoded_values) catch |err| {
