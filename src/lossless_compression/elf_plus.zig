...
//! binary mantissa, producing a near-equal v' with many trailing zeros. v' is then XOR-encoded
//! against the previous v' (four cases by leading/trailing-zero buckets); the decoder recovers
//! v from v' by rounding up to beta_star decimal digits. The bit-level layout, decimal-precision
//! tables, and recovery formulas follow the authors' reference Java implementation.
...
/// (1, 2, or 6 bits) followed by the XOR_cmp encoding of v'. If an error occurs it is returned.
...
pub fn compress(
    ...
    for (uncompressed_values[1..]) |value| {
        const erase_result = try eraseValue(&bit_writer, value, last_beta_star);
        last_beta_star = erase_result.new_last_beta_star;
        try xorCompressOther(&bit_writer, erase_result.v_prime_bits, &xor_state);
    }
...
pub fn decompress(
    ...
    while (decompressed_values.items.len < value_count) {
        ...
        //   "0"      (1 bit)  -> erased, beta_star reused from previous
        //   "10"     (2 bits) -> no erase
        //   "11"+beta_star (6 bits) -> erased, new beta_star (4 bits follow the 2-bit marker)
        const first_marker_bit = bit_reader.readBitsNoEof(u1, 1) catch return Error.ByteStreamError;

        if (first_marker_bit == 0) {
            // Case 0: erased, reuse beta_star. A well-formed stream guarantees last_beta_star is
            // non-null here (the encoder only emits this case when a previous case-11 set it).
...
/// Returns beta (decimal significand count) of |v|: how many significant decimal digits the
/// value has after stripping trailing zeros. Examples:
///   getSignificantCount(3.17,   sp=0,  ...) = 3   ("3", "1", "7")
///   getSignificantCount(0.0314, sp=-2, ...) = 3   ("3", "1", "4")
///   getSignificantCount(5.20,   sp=0,  ...) = 2   (trailing zero stripped -> "5", "2")
///
/// `v_abs` is the magnitude, `sp` its SP (from getSP), and `last_beta_star` the beta_star of the
/// previous erased value (null on the first encoded value). Algorithm: walk i upward to the first
/// power of ten that makes v * 10^i an exact integer, then strip trailing zeros; beta = sp + i + 1.
/// last_beta_star seeds the starting i so consecutive same-precision values skip early iterations.
/// Returns 17 (the f64 decimal-precision ceiling) for values with no clean short decimal form -
/// the caller treats that as "give up, no erase".
fn getSignificantCount(v_abs: f64, sp: i16, last_beta_star: ?u8) u8 {
    // Pick a starting i based on what we know about the previous value's beta_star.
    ...
    var i: i32 = blk: {
        if (last_beta_star) |bs| {
            if (bs != 0) {
                ...
                const candidate = @as(i32, bs) - @as(i32, sp) - 1;
                break :blk @max(candidate, 1);
            }
            // bs == 0 (previous was a 10^-i corner case): no useful precision hint.
            break :blk if (sp >= 0) 1 else -@as(i32, sp);
        }
        // No previous value: start at the maximum possible beta (17 digits).
        break :blk 17 - @as(i32, sp) - 1;
    };

    // Clamp i >= 1: the formula above can produce 0 or negative i for very large sp
    // (e.g. v >= 1e17). Those values fail the eraser's profitability check anyway; we
    // just need to avoid calling get10iP with a negative argument.
    if (i < 1) i = 1;
    ...
    var temp_long: i64 = if (temp < 0x1p63) @intFromFloat(temp) else 0;

    var iterations: u8 = 0;
    // Cast temp_long back to f64 for an f64 == f64 check. If the cast loses precision (the true
    // integer doesn't fit losslessly in f64), the loop continues - the precision check below
    // catches that case.
    ...
    // Example: v = 5.20 -> temp_long = 520, strip one zero -> temp_long = 52, beta = 2.
    while (i > 0 and @rem(temp_long, 10) == 0) {
        i -= 1;
        temp_long = @divTrunc(temp_long, 10);
    }
...
/// from beta_star and a fresh SP(v') computation. `last_beta_star` is a hint that speeds up the
/// beta iteration; pass null on the first value of the stream.
/// Caller must ensure v_abs is finite and > 0.
fn computeAlphaAndBetaStar(v_abs: f64, last_beta_star: ?u8) struct { alpha: i32, beta_star: u8 } {
    const sp_and_flag = getSPAnd10iNFlag(v_abs);
    const beta = getSignificantCount(v_abs, sp_and_flag.sp, last_beta_star);
...
/// Layer 1 (Eraser) encoder. Writes the 1-, 2-, or 6-bit prefix marker for one value to
/// `bit_writer`, decides whether to erase mantissa noise, and returns (v_prime_bits,
/// new_last_beta_star) for the caller to feed into Layer 2 (XOR_cmp) and thread into the
/// next call.
///
...
///
/// `last_beta_star` is the beta_star of the most recent erased value (null on first call,
/// preserved across no-erase values). The caller threads the returned new_last_beta_star into
/// the next invocation.
///
/// NaN handling differs from the Java reference: we preserve NaN payload bits (route through
/// no-erase) rather than canonicalizing to 0x7ff8000000000000, so the codec is bit-exact
/// lossless on every input - matching chimp64/chimp128/gorilla/elf.
fn eraseValue(
    bit_writer: *shared_structs.BulkBitWriter,
    v: f64,
    last_beta_star: ?u8,
) Error!struct { v_prime_bits: u64, new_last_beta_star: ?u8 } {
    const v_bits: u64 = @bitCast(v);

    // Special values: 0, +/-inf, NaN. Skip the decimal-precision machinery and pass raw bits
    // through XOR_cmp. last_beta_star is preserved so the next decimal value can still reuse it.
    if (v == 0.0 or std.math.isInf(v) or std.math.isNan(v)) {
        try bit_writer.writeBits(@as(u2, 0b10), 2);
        return .{ .v_prime_bits = v_bits, .new_last_beta_star = last_beta_star };
    }
    ...
    const v_abs = @abs(v);
    const ab = computeAlphaAndBetaStar(v_abs, last_beta_star);

    // Bail to no-erase if alpha is outside the useful range. Normally alpha equals the scale i
    // (>= 0), so these guards only catch the extremes where the significant-digit search saturated:
    ...
    // If not, "erasing" wouldn't change v_bits - route to no-erase.
    ...
    const v_prime_bits: u64 = mask & v_bits;
    ...
    if (last_beta_star) |lbs| {
        if (lbs == ab.beta_star) {
            ...
            return .{ .v_prime_bits = v_prime_bits, .new_last_beta_star = last_beta_star };
        }
    }
    ...
    return .{ .v_prime_bits = v_prime_bits, .new_last_beta_star = ab.beta_star };
}

/// Layer 1 (Restorer) decoder. Given an erased value `v_prime` (output of XOR_dcmp) and the
/// beta_star from the bitstream, returns the original v.
///
/// Two recovery paths:
...
test "elf_plus roundtrips decimal-originated values" {
    // Elf+'s bread and butter: sensor-style values with limited decimal precision exercise
    // the erase path (beta_star in [1, 4]) plus the pow10 corner case.
...
test "elf_plus roundtrips beta_star transitions" {
    ...
    // time - exercises the encoder's beta_star update logic and decoder's lastBetaStar tracking.
...
test "elf_plus roundtrips integer values" {
    ...
    // Verifies the no-erase guards in eraseValue don't break integer round-trips.
...
test "elf_plus compresses repeated values below raw size" {
    // A constant signal maximally exercises beta_star reuse + XOR_cmp case 01 (xor = 0):
...
test "elf_plus compresses decimal data below raw size" {
    // Sensor-style values with consistent 2-decimal precision - Elf+'s sweet spot. The eraser
    // strips noise bits and beta_star reuse keeps the per-value overhead at ~1 bit.
    const allocator = testing.allocator;
